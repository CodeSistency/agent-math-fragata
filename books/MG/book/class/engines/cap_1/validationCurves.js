...
function defAddCurv(def, board, refArtifact, points = false) {
  ...
  let curvePoints = !points
    ? def.points[def.points.length - 1].map((e) => e[0])
    : points;
  let pointsTypes = !points
    ? def.points[def.points.length - 1].map((e) => e[1])
    : points;
        ...
        for (var i = rangFilterCopy.length - 1; i >= 0; i--) {
          const match = interval.findIndex((intervalY, k) => {
            ...
            return (
              ...
              point2 &&
              (Array.isArray(y1) ||
                defInterPoint(rangFilterCopy[i][0], y1, noise)) &&
              (Array.isArray(y2) ||
                defInterPoint(rangFilterCopy[i][1], y2, noise))
            );
          });
          if (match != -1) {
...
function defModsXY(def, modeMod, value, option, m = false) {
  ...
  let mach = axie.find(
    (p) => p[2]._value.replace(new RegExp("[' ']", 'g'), '') === value,
  );
  if (mach[1]) {
...
function defCompare(def, board, curves, compares, xMod = false, yMod = false) {
  if (curves.length == 0) {
    return false;
  }
...
function curvesCleanData(def, artifact, points, yellow) {
  ...
  if (yellow) {
    ...
    def.typeArtifact == 'Evaluation'
      ? (auxResult.typeArtifact = 'Evaluation')
...
function validateCotePotitionValue(cote, condition) {
  const value =
    condition.value && condition.input
      ? cleamText(cote[2]._value) == cleamText(condition.value)
      : true;
  ...
  if (typeof condition.position === 'boolean' && condition.input) {
    resp =
      cleamText(cote[2]._value).search(
        condition.position ? regexPositive : regexNegative,
      ) != -1 &&
      (condition.position ? Number(cote[1].Y()) > 0 : Number(cote[1].Y()) < 0);
  } else {
    resp = condition.position
      ? defInterPoint(Number(condition.position), Number(cote[1].Y()), 0.1)
      : true;
    if (resp && condition.interval) {
      resp =
        condition.interval[0] < Number(cote[1].Y()) &&
        condition.interval[1] > Number(cote[1].Y());
    }
  }
  return resp && value;
}
...
function coteView(params = {}) {
  ...
  const input = params.input
    ? params.board.create('input', [params.x ?? 0, params.y ?? 0, ' ', ''], {
        layer: 0,
        name: ' ',
        cssStyle: 'width: 30px;',
      })
  ...
  params.board.on('move', function () {
    ...
    if (lineRef) {
      lineRef.point1.setPositionDirectly(JXG.COORDS_BY_USER, [
        params.y ? 8 : point.X(),
        params.x ? 8 : point.Y(),
      ]);
      lineRef.point2.setPositionDirectly(JXG.COORDS_BY_USER, [
        params.y ? -8 : point.X(),
        params.x ? -8 : point.Y(),
      ]);
    }
...
function defValidation(def, board, refArtifact) {
  ...
  if (def.statusValidate && !def.change) {
    defAlerts({
      def,
      id: refArtifact,
      text: 'No hay cambios en la Respuesta',
      type: 3,
    });
    return;
  }
  ...
  if (
    def.cote.special.length != 0 ||
    0 != allPoints[0].length ||
    0 != def.curves.length ||
    def.cote.top.length != 0 ||
    def.cote.bottom.length != 0
  ) {
    if (
      def.yellow == false &&
      scaleY &&
      ...
      if (twoBreakPoints(allDom)) {
        ...
        if (conditions.oneTwentySeven) {
          respneTwentySeven = defOneTwentySeven(
            def,
            board,
            curves,
            lastPoints,
          ...
          if (minResp && maxMin) {
            if (conditions.min) {
              ...
              if (minResp && conditions.min.global) {
                minResp = defValMaxMinGlobal(def, conditions.min, maxMin, 1);
              }
              if (minResp && conditions.min.minLocal) {
                minResp = defValMaxMinLocal(
                  def,
            ...
            if (conditions.max != undefined) {
              ...
              if (maxResp && conditions.max.global != undefined) {
                maxResp = defValMaxMinGlobal(def, conditions.max, maxMin, 2);
              }
              if (maxResp && conditions.max.maxLocal != undefined) {
                maxResp = defValMaxMinLocal(
                  def,
        ...
        if (
          ...
          if (!respParallels) {
            listError +=
              ' ' + (conditions.parallels.text ?? ' Paralelo al eje,');
          }
          if (!respConex) {
      ...
      if (def.yellow == true) {
        defAlerts({
          def,
          id: refArtifact,
          text: 'Tu respuesta sera tomada en cuenta GRACIAS',
        });
      } else {
        if (!scaleX && !scaleY) {
          defAlerts({
            def,
            id: refArtifact,
            text: 'Revisa la escala X y Y agrega solo los puntos necesarios',
            type: 2,
          });
        } else if (!scaleX) {
          defAlerts({
            def,
            id: refArtifact,
            text: 'Revisa la escala X agrega solo los puntos necesarios',
            type: 2,
          });
        } else if (!scaleY) {
          defAlerts({
            def,
            id: refArtifact,
            text: 'Revisa la escala Y agrega solo los puntos necesarios',
            type: 2,
          });
        } else if (!modCoteTop) {
          defAlerts({
            def,
            id: refArtifact,
            text: 'Revisa la creación de la cota superior.',
            type: 2,
          });
          ...
          defAlerts({
            def,
            id: refArtifact,
            text: 'Revisa la creación de la cota inferior.',
            type: 2,
          });
          ...
          defAlerts({
            def,
            id: refArtifact,
            text: 'Revisa la creación de la cota no cumple el intervalo.',
            type: 2,
          });
          ...
          defAlerts({
            def,
            id: refArtifact,
            text: 'Revisa la creación de la cota inferior e superior.',
            type: 2,
          });
        }
        def.dataInteraction.incorrect = 1;
  ...
  if (
    ...
    Android.sendData(
      JSON.stringify(curvesCleanData(def, refArtifact, def.points, def.yellow)),
    );
    gTime(def, false);
    def.statusValidate = false;
  } else {
  ...
  if (infinities[0].id === pointsResult.id) {
    ...
    const point = board.create(
      ...
      [
        function () {
          if (
            newCurves.points[mode ? 18 : newCurves.points.length - 18] !=
            undefined
          ) {
            return newCurves.points[
              mode ? 18 : newCurves.points.length - 18
            ].usrCoords.slice(1)[0];
          }
        },
        function () {
...
function defValidateScale(cond, userScale, option = true) {
  ...
  let scaleMod = userScale
    .map((x) => [x[1], x[2]._value.replace(/[ ]/, '')])
    .filter((p1) => {
      return !(
        defInterPoint(option ? p1[0].X() : p1[0].Y(), 0) && p1[1] == '0'
      );
    });
  scaleMod.sort((p1, p2) => {
    if (option) {
