...
function cDefValidation(def, refKey, board) {
  ...
  if (!def.statusValidate && def.change) {
    ...
    if (def.debug) {
      console.warn('{{{{{{{{{ DEBUG }}}}}}}}}');
      console.table(cleanDataCartesian(def, refKey));

    }/* else {

        
          if (typeof Android !== 'undefined' && typeof Android.sendData === 'function') {
             Android.sendData(JSON.stringify(cleanDataCartesian(def, refKey)));
             ...
             sendData(cleanDataCartesian(def, refKey));
          }
      }; */
...
function cleanDataCartesian(def, refKey) {
  let auxResult = {};
